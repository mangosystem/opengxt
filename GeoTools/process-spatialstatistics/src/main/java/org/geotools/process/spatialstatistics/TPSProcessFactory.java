 ...
 * @source $URL$
 */
public class TPSProcessFactory extends SpatialStatisticsProcessFactory {
    protected static final Logger LOGGER = Logging.getLogger(TPSProcessFactory.class);

    private static final String PROCESS_NAME = "TPS";

    /*
     * TPS(SimpleFeatureCollection inputFeatures, String inputField, Double cellSize, ReferencedEnvelope extent): GridCoverage2D
     */

    public TPSProcessFactory() {
        super(new NameImpl(NAMESPACE, PROCESS_NAME));
    }

    @Override
    public Process create() {
        return new TPSProcess(this);
    }

    @Override
    public InternationalString getTitle() {
        return getResource("TPS.title");
    }

    @Override
    public InternationalString getDescription() {
        return getResource("TPS.description");
    }
    ...
    public static final Parameter<Double> cellSize = new Parameter<Double>("cellSize",
            Double.class, getResource("TPS.cellSize.title"),
            getResource("TPS.cellSize.description"), false, 0, 1, Double.valueOf(0.0), null);

    /** extent */
    public static final Parameter<ReferencedEnvelope> extent = new Parameter<ReferencedEnvelope>(
            "extent", ReferencedEnvelope.class, getResource("TPS.extent.title"),
            getResource("TPS.extent.description"), false, 0, 1, null, null);
    ...
    public static final Parameter<GridCoverage2D> RESULT = new Parameter<GridCoverage2D>("result",
            GridCoverage2D.class, getResource("TPS.result.title"),
            getResource("TPS.result.description"));
