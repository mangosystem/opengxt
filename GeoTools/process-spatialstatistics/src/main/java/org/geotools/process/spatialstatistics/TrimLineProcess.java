...
public class TrimLineProcess extends AbstractStatisticsProcess {
    ...
    public Map<String, Object> execute(Map<String, Object> input, ProgressListener monitor)
        ...
        SimpleFeatureCollection inputFeatures = (SimpleFeatureCollection) Params.getValue(input,
                TrimLineProcessFactory.lineFeatures, null);
        if (inputFeatures == null) {
            throw new NullPointerException("inputFeatures parameter required");
        }

        Double dangleLength = (Double) Params.getValue(input, TrimLineProcessFactory.dangleLength,
        ...
        Boolean deleteShort = (Boolean) Params.getValue(input, TrimLineProcessFactory.deleteShort,
                TrimLineProcessFactory.deleteShort.sample);

        // start process
        SimpleFeatureCollection resultFc = null;
        try {
            TrimLineOperation operation = new TrimLineOperation();
            resultFc = operation.execute(inputFeatures, dangleLength, deleteShort);
        } catch (IOException e) {
