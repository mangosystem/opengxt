 ...
 * @author Minpa Lee
 * @since 1.0
 * @version $Id: SSStyleBuilder.java 1 2011-09-01 11:22:29Z minpa.lee $
 */
public class SSStyleBuilder {
    ...
    public SSStyleBuilder(SimpleFeatureType simpleFeatureType) {
        this.featureType = simpleFeatureType;
        this.featureType.getGeometryDescriptor().getLocalName();
        this.lineStroke = sf.createStroke(ff.literal(LINE_COLOR), ff.literal(LINE_WIDTH));
    }
    ...
    public Style getLISAStyle(String propertyName) {
        ...
        final Color[] colorList = { new Color(255, 99, 71), new Color(250, 250, 210),
                new Color(100, 149, 237), new Color(216, 191, 216), Color.WHITE };

        Stroke lineStroke = sf.createStroke(ff.literal(LINE_COLOR), ff.literal(LINE_WIDTH));
        FeatureTypeStyle fts = sf.createFeatureTypeStyle();
        fts.setName(styleName);
        ...
        for (int k = 0; k < classValues.length; k++) {
            ...
            Rule rule = sf.createRule();
            Filter filter = ff.equal(ff.property(propertyName), ff.literal(classValues[k]), false);
            rule.setFilter(filter);
