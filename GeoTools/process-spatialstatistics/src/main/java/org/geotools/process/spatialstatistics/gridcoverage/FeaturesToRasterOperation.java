...
import org.geotools.process.spatialstatistics.gridcoverage.PointToRasterOperation.AssignmentType;
...
public class FeaturesToRasterOperation extends RasterProcessingOperation {
    ...
    public GridCoverage2D execute(SimpleFeatureCollection inputFeatures, Number gridValue) {
        ...
        if (shapeType == SimpleShapeType.POINT) {
            PointToRasterOperation pointToRaster = new PointToRasterOperation();
            pointToRaster.setOuptputTypeName(getOuptputTypeName());
            pointToRaster.setOutputDataStore(getOutputDataStore());
    ...
    public GridCoverage2D execute(SimpleFeatureCollection inputFeatures, String valueField) {
        ...
        if (shapeType == SimpleShapeType.POINT) {
            ...
            return pointToRaster.execute(inputFeatures, valueField, AssignmentType.MOST_FREQUENT);
        }
        ...
        try {
            ...
            while (featureIter.hasNext()) {
                ...
                if (isNuemricField) {
                    ...
                    int featureID = FeatureTypes.getFID(feature);
