...
/**
 * The crop operation is responsible for selecting geographic subarea of the source coverage. The CoverageCrop operation does not merely wrap the JAI
 * Crop operation but it goes beyond that as far as capabilities.
 * 
 * <p>
 * The key point is that the CoverageCrop operation aims to perform a spatial crop, i.e. cropping the underlying raster by providing a spatial
 * {@link Envelope} (if the envelope is not 2D only the 2D part of it will be used). This means that, depending on the grid-to-world transformation
 * existing for the raster we want to crop, the crop area in the raster space might not be a rectangle, hence JAI's crop may not suffice in order to
 * shrink the raster area we would obtain. For this purpose this operation make use of either the JAI's Crop or Mosaic operations depending on the
 * conditions in which we are working.
 * 
 * 
 * <p>
 * <strong>Meaning of the ROI_OPTIMISATION_TOLERANCE parameter</strong> <br>
 * In general if the grid-to-world transform is a simple scale and translate using JAI's crop should suffice, but when the g2w transform contains
 * rotations or skew then we need something more elaborate since a rectangle in model space may not map to a rectangle in raster space. We would still
 * be able to crop using JAI's crop on this polygon bounds but, depending on how this rectangle is built, we would be highly inefficient. In order to
 * overcome this problems we use a combination of JAI's crop and mosaic since the mosaic can be used to crop a raster using a general ROI instead of a
 * simple rectangle. There is a negative effect though. Crop would not create a new raster but simply forwards requests back to the original one (it
 * basically create a viewport on the source raster) while the mosaic operation creates a new raster. We try to address this trade-off by providing
 * the parameter {@link Crop#ROI_OPTIMISATION_TOLERANCE}, which basically tells this operation "Use the mosaic operation only if the area that we
 * would load with the Mosaic is strictly smaller then (ROI_OPTIMISATION_TOLERANCE)* A' where A' is the area of the polygon resulting from converting
 * the crop area from the model space to the raster space.
 * 
 * <p>
 * <strong>ROI</strong><br>
 * By providing a ROI parameter, the coverage can be cropped by any set of polygons, even disjuncted ones. When the ROI is provided, the JAI's Mosaic
 * operation will be used. <br>
 * At least one between <i>Envelope</i> and <i>ROI</i> must be provided. If both of them are provided, the resulting area will be the intersection of
 * them. <br>
 * ROI geometries must be in the same CRS as the source coverage. <br>
 * ROI must be any among a {@link com.vividsolutions.jts.geom.Polygon}, a {@link MultiPolygon}, or a {@link GeometryCollection} of the two.
 * 
 * <p>
 * <strong>NOTE</strong> that in case we will use the Mosaic operation with a ROI, such a ROI will be added as a synthetic property to the resulting
 * coverage. The key for this property will be GC_ROI and the type of the object {@link Polygon}.
 * 
 * 
 * @source $URL: http://svn.osgeo.org/geotools/branches/2.7.x/modules/library/coverage/src/main/java/
 *         org/geotools/coverage/processing/operation/Crop.java $
 * @todo make this operation more t,z friendly
 * @version $Id: Crop.java 37514 2011-06-27 10:44:57Z aaime $
 ...
 */
public class RasterCropOperation extends Operation2D {
    ...
    public static final ParameterDescriptor<Envelope> CROP_ENVELOPE = new DefaultParameterDescriptor<Envelope>(
            Citations.GEOTOOLS, PARAMNAME_ENVELOPE, Envelope.class, // Value class
    ...
    public static final ParameterDescriptor<Geometry> CROP_ROI = new DefaultParameterDescriptor<Geometry>(
            Citations.JAI, PARAMNAME_ROI, Geometry.class, // Value class
            null, // Array of valid values
            null, // Default value
            null, // Minimal value
            null, // Maximal value
    ...
    /**
     * The parameter descriptor used to tell this operation to optimize the crop using a Mosaic in where the area of the image we would not load is
     * smaller than ROI_OPTIMISATION_TOLERANCE*FULL_CROP.
     */
    public static final ParameterDescriptor<Double> ROI_OPTIMISATION_TOLERANCE = new DefaultParameterDescriptor<Double>(
            Citations.GEOTOOLS, PARAMNAME_ROITOLERANCE, Double.class, // Value class
            null, // Array of valid values
            0.6, // Default value
            0.0, // Minimal value
            1.0, // Maximal value
     ...
     */
    public RasterCropOperation() {
        super(new DefaultParameterDescriptorGroup(Citations.GEOTOOLS, "CoverageCrop",
                new ParameterDescriptor[] { SOURCE_0, CROP_ENVELOPE, CROP_ROI,
    ...
    public GridCoverage2D execute(GridCoverage2D inputCoverage, ReferencedEnvelope extent) {
        ...
        return (GridCoverage2D) doOperation(param, null);
    }
    ...
    public Coverage doOperation(ParameterValueGroup parameters, Hints hints) {
        ...
        if (sourceParameter == null || !(sourceParameter.getValue() instanceof GridCoverage2D)) {
            throw new CannotCropException(Errors.format(ErrorKeys.NULL_PARAMETER_$2, "Source",
        ...
        if ((envelopeParameter == null || envelopeParameter.getValue() == null)
            ...
            throw new CannotCropException(Errors.format(ErrorKeys.NULL_PARAMETER_$2,
                    PARAMNAME_ENVELOPE, GeneralEnvelope.class.toString()));
        ...
        // Check crop ROI
        try {
            cropRoi = IntersectUtils.unrollGeometries((Geometry) roiParameter.getValue()); // may
            // throw
            // if
            // format
            // not
            ...
            throw new CannotCropException(Errors.format(ErrorKeys.ILLEGAL_ARGUMENT_$2,
                    PARAMNAME_ROI, ex.getMessage()), ex);
        }

        // Setting a GeneralEnvelope from ROI if needed
        if (cropRoi != null && cropEnvelope == null) {
            Envelope e2d = JTS.getEnvelope2D(cropRoi.getEnvelopeInternal(),
        ...
        CoordinateReferenceSystem destinationCRS = destinationEnvelope
        ...
        // intersect
        if (cropRoi != null) {
            final Geometry jis = JTS.toGeometry(intersectionEnvelope.toRectangle2D(),
                    cropRoi.getFactory());
        ...
        //
        // //
        final AffineTransform sourceCornerGridToWorld = (AffineTransform) ((GridGeometry2D) source
                .getGridGeometry()).getGridToCRS(PixelInCell.CELL_CORNER);
        ...
        if (cropRoi != null || !intersectionEnvelope.equals(sourceEnvelope, tolerance / 2.0, false)) {
            ...
            return buildResult(cropEnvelope, cropRoi, roiTolerance,
                    (hints instanceof Hints) ? (Hints) hints : new Hints(hints), source,
     ...
     */
    private static GridCoverage2D buildResult(final GeneralEnvelope cropEnvelope,
            final Geometry cropROI, final double roiTolerance, final Hints hints,
            final GridCoverage2D sourceCoverage, final AffineTransform sourceGridToWorldTransform) {
        ...
        //
        final boolean isSimpleTransform = CoverageUtilities.isSimpleGridToWorldTransform(
                sourceGridToWorldTransform, EPS);
        ...
        try {
            ...
            final double noDataValue = RasterHelper.getNoDataValue(sourceCoverage);
            ...
            if (cropROI != null) {
                ...
                Envelope2D e2d = JTS.getEnvelope2D(intersection.getEnvelopeInternal(),
            ...
            //
            // //
            final Rectangle2D finalRasterAreaDouble = XAffineTransform.transform(
            ...
            if (!isSimpleTransform || cropROI != null) {
                ...
                rasterSpaceROI = FeatureUtilities.convertPolygonToPointArray(modelSpaceROI,
                        ProjectiveTransform.create(sourceWorldToGridTransform), points);
                if (rasterSpaceROI == null || rasterSpaceROI.getBounds().isEmpty())
                    if (finalRasterArea.isEmpty())
                        throw new CannotCropException(Errors.format(ErrorKeys.CANT_CROP));
                final boolean doMosaic = decideJAIOperation(roiTolerance,
                        rasterSpaceROI.getBounds2D(), points);
                if (doMosaic || cropROI != null) {
                    // prepare the params for the mosaic
                    final ROI[] roiarr;
                    try {
                        if (cropROI != null) {
                            final Shape cropRoiLS2 = new LiteShape2(cropROI,
                                    ProjectiveTransform.create(sourceWorldToGridTransform), null,
                                    false);
                    ...
                    // 실제 NoData 지역임에도 불구하고 0값으로 설정되는 오류
                    // pbj.add(CoverageUtilities.getBackgroundValues(sourceCoverage));

                    double[] background = CoverageUtilities.getBackgroundValues(sourceCoverage);
                    background[0] = noDataValue;
                    pbj.add(background);
            ...
            GridGeometry2D envelope = new GridGeometry2D(new GridEnvelope2D(
                    croppedImage.getBounds()),
                    sourceGridGeometry.getGridToCRS2D(PixelOrientation.CENTER),
                    sourceCoverage.getCoordinateReferenceSystem());
            GridSampleDimension[] bands = actionTaken == 1 ? null : sourceCoverage
                    .getSampleDimensions().clone();
            GridCoverage[] sources = new GridCoverage[] { sourceCoverage };

            Map<CharSequence, Object> properties = new HashMap<CharSequence, Object>();
            ...
            return factory.create(name, croppedImage, envelope, bands, sources, properties);
     ...
     */
    private static boolean decideJAIOperation(final double roiTolerance,
            final Rectangle2D finalGridRange, final List<Point2D> points)
        ...
        final double roiArea = Math.abs(FeatureUtilities.area((Point2D[]) points
                .toArray(new Point2D[] {})));
