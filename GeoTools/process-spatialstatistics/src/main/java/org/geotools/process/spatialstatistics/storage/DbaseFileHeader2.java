...
public class DbaseFileHeader2 extends DbaseFileHeader {
        ...
        if (tempFieldName.length() > 10) {
            ...
            if (logger.isLoggable(Level.WARNING)) {
                logger.warning("FieldName " + inFieldName
                        + " is longer than 10 characters, truncating to " + tempFieldName);
            }
        }
        ...
        if ((inFieldType == 'C') || (inFieldType == 'c')) {
            ...
            if (inFieldLength > 254) {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("Field Length for " + inFieldName + " set to " + inFieldLength
            ...
            if (inDecimalCount < 0) {
                if (logger.isLoggable(Level.FINE)) {
                    logger.fine("Field Decimal Position for " + inFieldName + " set to "
                            + inDecimalCount + " Setting to 0 no decimal data will be saved.");
                }
                tempFieldDescriptors[fields.length].decimalCount = 0;
            }
            if (inDecimalCount > inFieldLength - 1) {
                if (logger.isLoggable(Level.WARNING)) {
                    logger.warning("Field Decimal Position for " + inFieldName + " set to "
            ...
            throw new DbaseFileException("Undefined field type " + inFieldType + " For column "
                    + inFieldName);
        }
        ...
        for (int i = 0, j = 0; i < fields.length; i++) {
            if (!inFieldName.equalsIgnoreCase(fields[i].fieldName.trim())) {
                ...
                if (i == j && i == fields.length - 1) {
                    System.err.println("Could not find a field named '" + inFieldName
                            + "' for removal");
                    return retCol;
                }
    ...
    public String toString() {
        ...
        for (int i = 0, ii = fields.length; i < ii; i++) {
            ...
            fs.append(f.fieldName + " " + f.fieldType + " " + f.fieldLength + " " + f.decimalCount
                    + " " + f.fieldDataAddress + "\n");
        }
