...
/**
 * Feature To Octagonal Envelope SimpleFeatureCollection Implementation
 * 
 * @author Minpa Lee, MangoSystem
 * 
 * @source $URL$
 */
public class OctagonalEnvelopeFeatureCollection extends GXTSimpleFeatureCollection {
    protected static final Logger LOGGER = Logging
            .getLogger(OctagonalEnvelopeFeatureCollection.class);

    private boolean singlePart;

    private SimpleFeatureType schema;

    public OctagonalEnvelopeFeatureCollection(SimpleFeatureCollection delegate, boolean singlePart) {
        super(delegate);
    ...
    public SimpleFeatureIterator features() {
        return new OctagonalEnvelopeFeatureIterator(delegate.features(), getSchema(), singlePart);
    }
    ...
    static class OctagonalEnvelopeFeatureIterator implements SimpleFeatureIterator {
        ...
        public OctagonalEnvelopeFeatureIterator(SimpleFeatureIterator delegate,
                ...
                if (attribute instanceof Geometry) {
                    ...
                    if (!singlePart && geometry.getNumGeometries() > 1) {
                        ...
                        for (int index = 0; index < geometry.getNumGeometries(); index++) {
                            ...
                            geomList.add(StaticGeometry.octagonalEnvelope(part));
                        }
                        attribute = geometry.getFactory().buildGeometry(geomList);
                    } else {
                        attribute = StaticGeometry.octagonalEnvelope(geometry);
                    }
                }
