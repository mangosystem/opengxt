...
public class MapUtils {
    ...
    public static SimpleFeatureCollection getFeatures(IMap map, String layerName) {
        for (ILayer layer : map.getMapLayers()) {
            if (layer.getName().equals(layerName) && layer.hasResource(FeatureSource.class)) {
                try {
                    SimpleFeatureSource sfc = (SimpleFeatureSource) layer.getResource(
                            FeatureSource.class, new NullProgressMonitor());
                    // apply selected features
                    // layer.getQuery(true).getFilter()
                    Filter filter = Filter.INCLUDE;
                    if (layer.getFilter() != Filter.EXCLUDE) {
                        filter = layer.getFilter();
                    }
                    return sfc.getFeatures(filter);
                } catch (IOException e) {
    ...
    public static void addGeometryToMap(IMap map, Geometry source, CoordinateReferenceSystem crs, String layerName) {
