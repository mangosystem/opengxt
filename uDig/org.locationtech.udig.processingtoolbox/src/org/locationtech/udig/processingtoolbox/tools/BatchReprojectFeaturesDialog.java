        ...
        try {
            ...
            for (TableItem item : inputTable.getItems()) {
                ...
                if (item.getChecked()) {
                    ...
                    File path = new File(folder, layer.getName() + ".shp"); //$NON-NLS-1$
                    if (path.exists()) {
                        if (MessageDialog.openQuestion(getShell(), windowTitle, MessageFormat.format(
                                Messages.BatchReprojectFeaturesDialog_Overwrite, layer.getName()))) {
                            if (MapUtils.confirmSpatialFile(path)) {
